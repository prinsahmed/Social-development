...
const ManageEvent = () => {
  ...
  useEffect(() => {
    axiosInstance.get(`/manage-event?email=${user?.email}`).then((res) => {
          ...
          {manageData.map((data) => {
            return (
              <motion.div
                ...
                key={data._id}
                className="feature-card flex items-center gap-x-4 mt-3 rounded-sm"
              >
                <img
                ...
                <div className="space-y-1">
                  ...
                  <Link
                    to={`/manage-event/update-event/${data._id}`}
                    className="btn"
                  >
                    Edit Event
                  </Link>
