...
const UpcomingEvent = () => {
  ...
  const fetchEvents = useCallback(
    debounce(async (searchTerm, category) => {
      try {
        ...
        if (category !== "Filter Events Type")
  ...
  return (
    ...
    >
      <title>Events</title>
      {events.length === 0 ? (
        <h2 className=" text-center underline text-4xl my-14 font-medium ">
          No Events Available.
        </h2>
      ) : (
        <h2 className="text-center underline text-4xl my-14 font-medium ">
      ...
      <div className="flex justify-between mb-10">
        <div className="text-xl font-medium">Events: ({events.length})</div>
        ...
        {events.map((event) => {
          return (
            <motion.div
              ...
              key={event._id}
              className="bg-white  feature-card shadow-lg rounded-lg overflow-hidden transition-transform duration-300"
            >
              <img
                src={event.photoURL}
                alt={event.title}
              ...
              <Link
                to={`/up-event/details/${event._id}`}
                className="mt-2 btn bg-blue-600 text-white py-2 rounded-lg hover:bg-blue-700 transition-colors"
              >
                View Event
              </Link>
            </motion.div>
